...
def scrape_yahoo(scrolldown=2000):
    ...
    articles = soup.find_all(
        'div', {'class': 'Cf', 'data-test-locator': False})
    ...
    for article in articles:
        try:
            ...
            if "hour" in time_text:
                ...
                ) - datetime.timedelta(hours=int(minutes_or_hours))
            elif "minute" in time_text:
                datetime_date = datetime.datetime.now(
                ) - datetime.timedelta(minutes=int(minutes_or_hours))

            article_list.append(
                Article(text, link, datetime_date, "yahoo finance"))
        except:
            pass
